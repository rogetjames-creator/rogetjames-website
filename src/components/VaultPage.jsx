  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-5xl w-full mx-20 md:mx-28 flex flex-col items-center">
        ...
        />
        {items[idx].name && (
          <p className="mt-5 font-detail text-xs text-cream/70 uppercase tracking-widest">
            {items[idx].name.replace(/\.[^.]+$/, "")}
          </p>
        )}
        {total > 1 && (
          <p className="font-detail text-[10px] text-cream/80 uppercase tracking-widest mt-2">
            {idx + 1} / {total}
          </p>
        )}
      </div>
  ...
  return (
    <section className="relative h-screen overflow-hidden bg-jet select-none">
      ...
      <div ref={titleRef} className="absolute bottom-0 left-0 px-8 md:px-16 pb-20 max-w-3xl">
        ...
        {projectTitle && (
          <p className="font-detail text-sm text-cream/70 uppercase tracking-[0.18em] opacity-0">
            {projectTitle}
          </p>
        )}
      </div>
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mt-4 px-1">
        <p className="font-detail text-xs text-cream/70 uppercase tracking-wider truncate">
...
function KeyPoints({ points }) {
  ...
  return (
    <section ref={sectionRef} className="max-w-6xl mx-auto px-6 py-14 border-b border-cream/[0.14]">
      <p className="font-detail text-[10px] text-cream/85 uppercase tracking-[0.22em] mb-8">Key Points</p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {points.map((point, i) => (
          <div
            key={i}
            className="kp-item flex gap-4 p-5 bg-cream/[0.07] border border-cream/[0.14] rounded-2xl hover:border-clay/25 transition-colors duration-300"
          >
...
function LinksSection({ links }) {
  ...
  return (
    <section ref={sectionRef} className="max-w-6xl mx-auto px-6 py-14 border-b border-cream/[0.14]">
      <p className="font-detail text-[10px] text-cream/85 uppercase tracking-[0.22em] mb-8">Links & Resources</p>
        ...
        {links.map((link, i) => {
          ...
          return (
            <a
              ...
              className="link-card group flex items-start justify-between p-5 bg-cream/[0.07] border border-cream/[0.14] rounded-2xl hover:border-clay/35 hover:bg-cream/[0.06] transition-all duration-200"
            >
              <div className="pr-3">
                ...
                {link.description && (
                  <p className="font-detail text-xs text-cream/70 mt-1.5 leading-relaxed">{link.description}</p>
                )}
              </div>
...
function DocumentsSection({ pdfs }) {
  return (
    <section className="max-w-6xl mx-auto px-6 py-14 border-b border-cream/[0.14]">
      <p className="font-detail text-[10px] text-cream/85 uppercase tracking-[0.22em] mb-8">Documents</p>
      <div className="space-y-3 max-w-xl">
        {pdfs.map((pdf, i) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-4 p-4 bg-cream/[0.07] border border-cream/[0.14] rounded-2xl hover:border-clay/35 hover:bg-cream/[0.06] transition-all duration-200 group"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="font-detail text-[10px] text-cream/85 uppercase tracking-wider mt-0.5">PDF</p>
            </div>
          </a>
        ))}
      </div>
...
function VaultContent({ clientData }) {
  ...
  return (
    <div className="min-h-screen bg-jet text-cream">
      ...
      <header
        ref={headerRef}
        className="fixed top-0 left-0 right-0 z-50 border-b border-cream/[0.14] bg-charcoal/92 backdrop-blur-md transition-all duration-400"
      ...
      >
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            <span className="font-detail text-[10px] text-cream/65 uppercase tracking-[0.2em] hidden sm:block truncate max-w-[200px]">
              {clientName}
            </span>
            <a
              href="/"
      ...
      {hasImages ? (
        ...
        <div className="h-56 flex flex-col justify-end px-8 md:px-16 pb-10 border-b border-cream/[0.14]">
          ...
          {projectTitle && (
            <p className="font-detail text-sm text-cream/75 uppercase tracking-[0.18em] mt-2">{projectTitle}</p>
          )}
        </div>
      )}

      {/* Project overview */}
      <section ref={overviewRef} className="max-w-6xl mx-auto px-6 py-14 border-b border-cream/[0.14]">
        <div className="flex flex-col md:flex-row md:items-start gap-10">
          <div className="flex-1">
            ...
            {projectDescription && (
              <p className="reveal font-detail text-sm text-cream/85 leading-relaxed max-w-2xl whitespace-pre-line">
                {projectDescription}
              </p>
            )}
          </div>
          {statusConfig && (
            <div className="reveal flex-shrink-0">
              <div className="inline-flex items-center gap-2.5 bg-cream/[0.04] border border-cream/[0.16] rounded-2xl px-5 py-3">
                <div className="w-2 h-2 rounded-full" style={{ background: statusConfig.dot }} />
                <div>
                  <p className="font-detail text-[9px] text-cream/65 uppercase tracking-[0.2em] mb-0.5">Status</p>
      ...
      {hasImages && (
        <section className="max-w-6xl mx-auto px-6 py-14 border-b border-cream/[0.14]">
          <p className="font-detail text-[10px] text-cream/85 uppercase tracking-[0.22em] mb-8">Project Images</p>
          <GallerySlideshow
            images={images}
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 text-center">
        <div className="max-w-xl mx-auto">
          <p className="font-detail text-[10px] text-cream/55 uppercase tracking-[0.25em] mb-3">Discover more</p>
        ...
        <div className="mt-14 pt-8 border-t border-cream/[0.06]">
          <p className="font-detail text-[9px] text-cream/45 uppercase tracking-widest">
  ...
  return (
    <div className="min-h-screen bg-jet flex items-center justify-center px-6">
      <div className="text-center max-w-sm">
        ...
        <p className="font-detail text-sm text-cream/80 leading-relaxed">
          This link is invalid or has expired. Please contact us to request a new one.
        </p>
        <a href="/" className="mt-8 inline-flex items-center gap-2 font-detail text-xs text-cream/70 uppercase tracking-[0.2em] hover:text-cream transition-colors">
  ...
  return (
    <div className="min-h-screen bg-jet flex items-center justify-center px-6">
      ...
      <div ref={formRef} className="relative w-full max-w-sm opacity-0">
        <div className="bg-white/10 border border-white/20 rounded-[2rem] p-8 backdrop-blur-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <input
                ...
                autoComplete="email"
                className="w-full bg-white/8 border border-white/20 focus:border-clay/65 rounded-2xl px-5 py-3.5 text-center font-detail text-cream placeholder:text-cream/85 outline-none transition-colors duration-200"
            ...
            <button
              ...
              className="w-full py-3.5 rounded-2xl bg-clay text-cream font-heading font-semibold text-sm tracking-wide hover:bg-clay-light transition-all duration-200 flex items-center justify-center gap-2"
            >
              {loading ? (
          ...
          <p className="font-detail text-[10px] text-cream/65 text-center mt-6">
...
function AdminPanel() {
    ...
    return (
      <div className="min-h-screen bg-jet flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          <div className="bg-white/10 border border-white/20 rounded-[2rem] p-8">
            <div className="text-center mb-8">
              <a href="/" className="inline-block font-heading font-bold text-cream text-xl tracking-tight">
                ROGET<span className="font-normal italic font-drama">james</span>
              </a>
            ...
            <form onSubmit={e => { e.preventDefault(); if (secret.trim()) setAuthed(true); }} className="space-y-4">
              <input
                ...
                className="w-full bg-white/8 border border-white/20 focus:border-clay/65 rounded-2xl px-5 py-3.5 text-center font-heading text-cream tracking-[0.15em] placeholder:text-cream/85 placeholder:font-detail placeholder:text-sm placeholder:tracking-widest outline-none transition-colors"
                style={{ caretColor: "#C45018" }}
              />
              <button
                type="submit"
  ...
  return (
    <div className="min-h-screen bg-jet px-6 py-16">
      <div className="max-w-lg mx-auto">
        <div className="mb-10">
          ...
          <p className="font-detail text-xs text-clay uppercase tracking-[0.25em]">Vault Admin · Send Invite</p>
          <p className="font-detail text-sm text-cream/70 mt-3 leading-relaxed">Send a client their private vault link. They must already have a row in Airtable with their email address.</p>
        </div>
        <div className="bg-white/10 border border-white/20 rounded-2xl p-8">
          <p className="font-detail text-[10px] text-cream/60 uppercase tracking-[0.2em] mb-5">Client Email</p>
          <form onSubmit={handleSend} className="space-y-4">
            <input
              ...
              className="w-full bg-white/8 border border-white/20 focus:border-clay/65 rounded-2xl px-5 py-3.5 font-detail text-cream placeholder:text-cream/85 outline-none transition-colors"
              style={{ caretColor: "#C45018" }}
            />
            <button
              type="submit"
              disabled={!inviteEmail.trim() || loading}
              className="w-full py-3.5 rounded-2xl bg-clay text-cream font-heading font-semibold text-sm tracking-wide hover:bg-clay-light transition-all flex items-center justify-center gap-2"
            >
              {loading ? (
          ...
          {result?.success && (
            <div className="mt-5 p-4 bg-green-500/10 border border-green-500/20 rounded-xl space-y-3">
              ...
              <p className="font-detail text-[10px] text-cream/65 uppercase tracking-wider">Vault URL</p>
              <p className="font-mono text-xs text-cream/85 break-all bg-cream/5 rounded-lg px-3 py-2">
                {result.vaultUrl}
              </p>
              <button
...
export default function VaultPage() {
  ...
  const DEV_DATA = {
    clientName: "Amanda Dwyer",
    projectTitle: "Garden Sculpture Commission",
    location: "Perth, WA",
    status: "In Progress",
    greeting: "Amanda, here's your private preview of the commission we've been developing together.",
    projectDescription: "A large-scale outdoor sculpture for the north garden — inspired by native banksia forms. Fabricated in 3mm Corten steel with a natural weathering finish.",
    images: [],
    keyPoints: ["Corten steel — develops a rich rust patina over 12–18 months", "Dimensions: 1800 × 600 × 400mm", "Installation included — concrete footing required", "Lead time: 8–10 weeks from approval"],
    links: [{ label: "ROGETjames Website", url: "/", description: "Explore the full collection" }],
    pdfs: [],
  };
  ...
  useEffect(() => {
    if (isDev) { setClientData(DEV_DATA); setStep("content"); return; }
