...
.fw-exit{display:flex;align-items:center;justify-content:center;width:38px;height:38px;flex:0 0 auto;border-radius:50%;background:rgba(20,20,20,.68);border:1px solid rgba(242,240,233,.42);color:#F2F0E9;cursor:pointer;text-decoration:none;backdrop-filter:blur(4px);transition:.25s}
...
.fw-nav{width:52px;height:52px;border-radius:50%;border:1.5px solid rgba(242,240,233,.4);background:rgba(20,20,20,.68);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);color:#F2F0E9;display:grid;place-items:center;cursor:pointer;transition:.3s}
...
const SITE_NAV_CURRENT = "/sculpture";
    ...
    <div className="fw-wrap">
          ...
          <div className="fw-subrail" ref={subrailRef} key={c.id}>
            {pieces.map((p, i) => (
              <div key={p.name + i} className={`fw-subcard ${i === pieceIdx ? "on" : ""} ${i === pieceFlash ? "flash" : ""}`}
                ...
                <img src={p.img} alt={pieceAlt(p.name, c.label, "laser cut metal garden sculpture")} />
              </div>
            ))}
          </div>
        )}
      ...
      {expanded && (
        <div className="fw-expand-overlay"
          ...
          onTouchStart={(e) => { expandTouchX.current = e.touches[0].clientX; expandTouchY.current = e.touches[0].clientY; expandSwiped.current = false; }}
          onTouchEnd={(e) => { const dx = e.changedTouches[0].clientX - expandTouchX.current; const dy = e.changedTouches[0].clientY - expandTouchY.current; if (Math.abs(dx) > 50 && Math.abs(dx) > Math.abs(dy) * 1.4) { expandSwiped.current = true; if (pieces.length > 1) expandNav(dx < 0 ? 1 : -1); } }}
        >
          {pieces.length > 1 && (
            <button
          ...
          <div className="fw-expand-imgwrap" onClick={(e) => e.stopPropagation()}>
            <img src={activePiece.img} alt={pieceAlt(activePiece.name, c.label, "laser cut metal garden sculpture")} className="fw-expand-img" />
